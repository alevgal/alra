...
add_action('wp_enqueue_scripts', function () {
    ...
    wp_add_inline_script('alra/vendor.js', asset('scripts/manifest.js')->contents(), 'before');
...
add_action('enqueue_block_editor_assets', function () {
    if ($manifest = asset('scripts/manifest.asset.php')->load()) {
        wp_enqueue_script('alra/vendor.js', asset('scripts/vendor.js')->uri(), ...array_values($manifest));
        wp_enqueue_script('alra/editor.js', asset('scripts/editor.js')->uri(), ['alra/vendor.js'], null, true);

        wp_add_inline_script('alra/vendor.js', asset('scripts/manifest.js')->contents(), 'before');
    }

    wp_enqueue_style('alra/editor.css', asset('styles/editor.css')->uri(), false, null);
}, 100);
